...
# ── API Key input ─────────────────────────────────────────────────────────────
with st.expander("⚙️ Setup: Enter your OpenAI API key", expanded=not st.session_state.vector_store_ready):
    ...
    if api_key and not st.session_state.vector_store_ready:
        if st.button("Connect and Build Culture Database"):
            try:
                with st.spinner("Connecting to OpenAI and building culture vector store..."):
                    ...
                    try:
                        chroma_client.delete_collection("ctm_culture")
                    except:
                        pass
                    ...
                    for i, (standard, desc) in enumerate(CULTURE_STANDARDS.items()):
                        doc = f"CTM Culture Standard: {standard}\n\nDescription: {desc}\n\nMission: {MISSION}"
                        documents.append(doc)
                        ids.append(f"standard_{i}")
                        metadatas.append({"standard_name": standard})
                ...
                st.success(f"Connected! All {len(CULTURE_STANDARDS)} CTM culture standards are indexed and ready.")
                st.rerun()
...
if st.session_state.vector_store_ready:
    ...
    additional_context = st.text_area(
        ...
        placeholder="E.g. 'The team just wrapped up a big project. Focus on celebrating the win while reinforcing accountability.'",
        height=100
    )
    ...
    if st.button("Generate content"):
        ...
        if not focus and standard_option == "Find best match automatically":
            st.warning("Please describe what you want the content to focus on, or select a specific standard.")
        else:
            with st.spinner("Finding best standard match, generating content, and running safety review..."):

                # RAG retrieval
                if standard_option == "Find best match automatically":
                    rag_results = collection.query(query_texts=[focus], n_results=1)
                    matched_standard = rag_results["metadatas"][0][0]["standard_name"]
...
Rules: Ground content in CTM's actual culture. Bold, real, next level. Never use dashes or em dashes. No filler. Write like a leader who believes every word. Content must be inclusive and appropriate for all employees."""
...
Generate the content now."""
                ...
                # Safety check
                safety_prompt = f"""Review this workplace communication content for: bias, legal risk in HR context, exclusionary language, off-brand tone, sensitive topics.
...
RECOMMENDATION: [one sentence]"""
    ...
    if st.session_state.generated_content:
        ...
        for i, item in enumerate(st.session_state.generated_content):
            with st.container():
                st.markdown(f'<span class="standard-badge">{item["standard"]}</span>&nbsp;&nbsp;<small style="color:#888;">{item["content_type"]} &nbsp;·&nbsp; {item["timestamp"]}</small>', unsafe_allow_html=True)

                st.markdown(f'<div class="result-box">{item["content"]}</div>', unsafe_allow_html=True)

                if item["safe"]:
                    st.markdown('<div class="safety-pass">✅ Safety review passed. Approved for human review before distribution.</div>', unsafe_allow_html=True)
                else:
...
⚠️ <strong>Human review required.</strong> All AI-generated content must be reviewed and approved by a human before distribution to staff. AI can make mistakes. You are the final decision maker.<br><br>
🔒 <strong>Privacy.</strong> Your API key is used only for this session and is never stored or shared.<br><br>
...
""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("**🔍 Smart matching**\n\nDescribe what you want in plain language. The AI finds the most relevant CTM culture standard automatically.")
    with col2:
        st.markdown("**✍️ On-brand content**\n\nEvery piece is grounded in CTM's actual culture standards, mission, and voice.")
    with col3:
        st.markdown("**🛡️ Safety review**\n\nEvery output is reviewed for bias, legal risk, and off-brand language before you see it.")
