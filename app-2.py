...
# ── Setup ─────────────────────────────────────────────────────────────────────
with st.expander("⚙️ Setup: Connect your OpenAI API key", expanded=not st.session_state.setup_complete):
    ...
    if api_key and not st.session_state.setup_complete:
        if st.button("Connect and build culture database"):
            try:
                with st.spinner("Connecting to OpenAI and embedding all 10 CTM culture standards..."):
                    client = OpenAI(api_key=api_key)

                    # Embed each culture standard using OpenAI embeddings
                    embeddings_store = {}
...
Generate the content now."""
                ...
                safety_response = client.chat.completions.create(
                    ...
                    max_tokens=200,
                    messages=[
                        {"role": "system", "content": "You are a responsible AI content safety reviewer for HR communications. Be thorough but not overly restrictive."},
                        {"role": "user", "content": f"""Review this workplace communication for: bias, legal risk in HR context, exclusionary language, off-brand tone, sensitive topics.
...
Powered by CTM Culture Workbook &nbsp;·&nbsp; OpenAI GPT-4o + Embeddings &nbsp;·&nbsp; VEAUX Professional Services
</div>
""", unsafe_allow_html=True)

else:
    st.info("Enter your OpenAI API key above and click Connect to get started.")
    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown("**🔍 Smart matching**\n\nDescribes what you want in plain language. AI finds the most relevant CTM standard automatically.")
    with col2:
        st.markdown("**✍️ On-brand content**\n\nGrounded in CTM's actual culture standards, mission, and voice.")
    with col3:
        st.markdown("**🛡️ Safety review**\n\nEvery output reviewed for bias, legal risk, and off-brand language before you see it.")
